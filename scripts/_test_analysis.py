...
_ap.add_argument("--no-snapshot", action="store_true",
                 help="（已废弃·兼容保留）原用于跳过数值快照断言；N3 起快照已改为区间/关系断言，本开关不再影响任何断言")
_args = _ap.parse_args()
NO_SNAPSHOT = _args.no_snapshot   # 兼容旧调用；不再有断言受其控制
...
# 4.5 逻辑一致：dy 分位低(贵) → 卖出锚贴近现价（N3：改区间断言，原精确快照 2.3±1.0 随行情漂移即红）
_ds = by_code["000922"]["anchors"]["dist_sell"]
check("000922 卖出锚距现价 ∈[-10%, +20%]", -10 <= _ds <= 20, f"dist_sell={_ds}%")
...
# ── T7 数值稳定性与同源一致性（N3：原「三档分数精确快照」→ 关系断言，避免行情漂移误报）──
print("\n── T7 数值稳定性与同源一致性 ──")


def recalc_score(v, pname):
    """按 analysis.presets 权重从 factors 分位重算（与前端 analysis.js scoreOf 同口径）"""
    w = analysis["presets"][pname]["B" if v["type"] == "股票" else "A"]
    present = [(k, wgt) for k, wgt in w.items() if v["factors"].get(k) and v["factors"][k]["pct"] is not None]
    if not present:
        return None
    return sum(v["factors"][k]["pct"] * wgt for k, wgt in present) / sum(wgt for _, wgt in present)


# 7.1 三档分数必须有限且落在 [0,100]（公式/权重被改坏会立刻暴露）
bad = []
for c in ("000922", "515180", "600036"):
    v = by_code.get(c)
    if not v:
        continue
    for pname in analysis["presets"]:
        s = recalc_score(v, pname)
        if s is None or not (0 <= s <= 100):
            bad.append((c, pname, None if s is None else round(s, 1)))
check("三档分数∈[0,100]（抽 3 标的）", not bad, f"异常: {bad[:3]}")
# 7.2 同源一致性：000922 与跟踪 ETF 515180 因子同源（仅 sent 不同）→ 同档分数差 ≤ 8pp
_diff = {p: round(abs(recalc_score(by_code["000922"], p) - recalc_score(by_code["515180"], p)), 1)
         for p in analysis["presets"]}
check("000922 vs 515180 同档分数差≤8pp", all(d <= 8 for d in _diff.values()), f"差异 {_diff}")
print("  （留痕：000922 三档分数 " + "，".join(f"{p} {recalc_score(by_code['000922'], p):.1f}"
      for p in analysis["presets"]) + "）")
