...
│   ├── _test_analysis.py    # ★ S1-S8 回归测试（65 项；CI 用 --no-snapshot 跑 63 项结构断言）
...
- **测试**（`_test_analysis.py`）：65 项断言（T1-T9 数据/结构/数值快照/边界/回测产物 + T10 池/产物/文件一致性关系断言），`python scripts/_test_analysis.py` 一键回归；前端纯函数 14 项单测（`node --test --test-isolation=none tests/frontend/analysis.test.mjs tests/frontend/reco.test.mjs`，覆盖权重表/分档边界/候选池构建/评分公式）+ 19 项浏览器矩阵（CloakBrowser）
