...
11 个视图的浏览器研究终端（hash 路由，视图容器常驻切换不销毁）：指数历史 / ETF历史 / 股票历史 / 成分股汇总 / 回测报告 / 组合回测 / 对比分析 / 轮动回测 / 信号扫描 / 智能推荐 / 我的持仓，深色金融科技风，红涨绿跌。
...
- **规则**：神火股份(000933) / 云铝股份(000807) / 电投能源(002128) 三只之间「卖高买低」——每交易日收盘取三只收盘价，`最高 − 最低 ≥ Δ`（或相对差 ≥ Δ%）且持仓不是最低价那只 → 次一交易日收盘（t+1，可切当日）卖出持仓买入最便宜的那只
- **区间可调**：任意起止日期（数据自电投能源上市 2007-04-18 起，默认近 5 年），改区间立即重算
- **胜率口径**：每次换仓 = 一次决策 —— 比较「换入那只」与「被卖出那只」在同一持仓区间的收益，换入更高即赢；末段未了结默认剔除（另给含未了结口径 + Wilson 95% 置信区间）
- **核心交付**：Δ 网格扫描（换仓次数 / 已了结 / 胜率＋95%CI / **平衡胜率** / **真实边际** / 保守边际 / 平均超额 / 达标交易日 / 近1年次数 / 策略收益 / 年化 / 回撤），点任意一行即用该 Δ 重算曲线与明细
- **选择结论卡**（自动、随区间与口径实时重算）：推荐换仓差价 Δ（含相对差换算）、出现次数（换仓次数 + 次/年 + 近1年 + 价差达标交易日占比）、胜率、**盈亏平衡胜率**、**真实边际**、平均超额、保守边际、触发年份分布，另给「高确信档」；选择规则 = 在「已了结 ≥12 次且近1年触发 ≥3 次」的档里取保守边际（95%CI 下界 − 平衡胜率）最大者（样本不足退到 ≥8 次并标注"仅作参考"）
- **口径**：不复权真实价做信号；含分红（除权派现给上一收盘持有者，现金不复投）；换仓扣双边成本（默认单边 0.05%，可调）；对照为三只买入持有与等权
- **实现**：浏览器直读 `/cache/股票_*.json` + `/cache/分红_*.json` 现算（零后端、零数据产物）；引擎 `web/js/views/rotation.js`（纯函数）单测 19 项 `tests/frontend/rotation.test.mjs`，浏览器冒烟 T14
